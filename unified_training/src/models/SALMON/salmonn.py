...
from ...utils.focal_loss import focal_cross_entropy
...
class SALMONN(nn.Module):
    ...
    def __init__(
        ...
        debug=True,  # Enable debug logs in __init__ by default
    ):
        super().__init__()

        self._debug = bool(debug)
        # When debug=True, log at INFO so messages are visible with default logging level
        _log = (lambda msg, *args, **kwargs: print(msg, *args, **kwargs)) if self._debug else (lambda msg, *args, **kwargs: None)

        _log("SALMONN __init__ started")
        _log(
            "Paths: llama=%s whisper=%s beats=%s speech_llama_proj=%s",
            llama_path, whisper_path, beats_path, speech_llama_proj_model,
        )
        _log(
            "Flags: freeze_whisper=%s whisper_unfreeze_last_n=%s whisper_attention_only=%s freeze_beats=%s",
            freeze_whisper, whisper_unfreeze_last_n_layers, whisper_unfreeze_attention_only, freeze_beats,
        )
        _log(
            "QFormer: use_speech_Qformer=%s num_speech_query_token=%s freeze=%s window_level=%s second_per_window=%s second_stride=%s",
            use_speech_Qformer, num_speech_query_token, freeze_speech_QFormer,
            window_level_Qformer, second_per_window, second_stride,
        )
        _log(
            "LoRA: lora=%s rank=%s alpha=%s dropout=%s",
            lora, lora_rank, lora_alpha, lora_dropout,
        )
        _log(
            "Prompts: multi_prompt=%s prompt_path=%s max_txt_len=%s low_resource=%s torch_dtype=%s",
            multi_prompt, prompt_path, max_txt_len, low_resource, torch_dtype,
        )
        _log(
            "Class weights: use_class_weights=%s bonafide_weight_multiplier=%s",
            use_class_weights, bonafide_weight_multiplier,
        )
        ...
        # Focal loss for LM (set in from_config from Model config)
        self.use_focal_loss = False
        self.focal_gamma = 2.0

        _log("Loading LLaMA tokenizer from %s", llama_path)
        logging.info('Loading LLaMA Tokenizer')
        self.llama_tokenizer = LlamaTokenizer.from_pretrained(llama_path, use_fast=False, local_files_only=False)
        self.llama_tokenizer.add_special_tokens({'pad_token': '[PAD]'})
        self.llama_tokenizer.padding_side = "right"
        _log("LLaMA tokenizer loaded: vocab_size=%d", len(self.llama_tokenizer))

        _log("Loading LLaMA model (low_resource=%s)", self.low_resource)
        ...
        if self.low_resource:
            ...
            self.llama_model = LlamaForCausalLM.from_pretrained(
                ...
                local_files_only=False,
                # vocab_size=len(self.llama_tokenizer)
                # old transformers version has no FA support for Llama
                # attn_implementation="flash_attention_2", # Enable Flash Attention
                # device='cuda',
            )

        # mean_resizing=False avoids slow covariance computation over full vocab (can appear to hang)
        self.llama_model.to('cuda')
        _log("Run resize")
        self.llama_model.resize_token_embeddings(
            len(self.llama_tokenizer), mean_resizing=False
        )
        n_llama_params = sum(p.numel() for p in self.llama_model.parameters())
        _log("LLaMA model loaded: %dM parameters", n_llama_params / 1e6)
        for name, param in self.llama_model.named_parameters():
            param.requires_grad = False
        logging.info('Loading LLaMA Done')

        if self.lora:
            _log("Applying LoRA: rank=%s alpha=%s dropout=%s", lora_rank, lora_alpha, lora_dropout)
            ...
            _log("LoRA applied in %.2f s", elapsed)
            self.llama_model.print_trainable_parameters()
            trainable = sum(p.numel() for p in self.llama_model.parameters() if p.requires_grad)
            _log("LoRA applied: %d trainable parameters", trainable)
            logging.info('LoRA Training')

        _log("Loading Whisper encoder from %s", whisper_path)
        ...
        self.speech_encoder = WhisperModel.from_pretrained(whisper_path, local_files_only=False).encoder
        self.ln_speech = nn.LayerNorm(self.speech_encoder.config.d_model)
        n_whisper_params = sum(p.numel() for p in self.speech_encoder.parameters())
        _log("Whisper encoder loaded: d_model=%d, %dM parameters", self.speech_encoder.config.d_model, n_whisper_params / 1e6)
        ...
        if self.beats_path:
            _log("BEATs path set: %s", self.beats_path)
            if not BEATS_AVAILABLE:
                ...
                _log("Loading BEATs from %s", self.beats_path)
            ...
            _log("BEATs not used (beats_path empty)")

        if self.use_speech_Qformer:
            speech_width = self.speech_encoder.config.d_model + (self.beats.cfg.encoder_embed_dim if self.beats_path else 0)
            _log("Initializing speech QFormer: num_query_token=%s speech_width=%s", num_speech_query_token, speech_width)
            if self.beats_path:
            ...
            n_qformer_params = sum(p.numel() for p in self.speech_Qformer.parameters()) + self.speech_query_tokens.numel()
            _log("Speech QFormer initialized: %dM parameters, freeze_speech_QFormer=%s", n_qformer_params / 1e6, freeze_speech_QFormer)
            ...
            _log("Creating speech_llama_proj: %d -> %d", self.speech_Qformer.config.hidden_size, self.llama_model.config.hidden_size)
        ...
        # prepare prompts
        _log("Preparing prompts: prompt_path=%s", prompt_path or "(none)")
        self.prompt_dict = {}
        if prompt_path:
            try:
            ...
            for task in raw_prompts.keys():
                ...
                _log("Loaded %d prompts for task '%s'", len(self.prompt_dict[task]), task)
            print("Loading training prompts done!")

        _log("SALMONN __init__ finished")
    ...
    def forward(self, samples, verbose=False):
        ...
        # calulate loss (per-sample for weighting in epoch)
        with self.maybe_autocast():
            ...
            # Per-token loss with reduction='none', then mean per sample -> (B,)
            logits_shifted = outputs.logits[:, :-1, :].reshape(-1, outputs.logits.size(-1))
            labels_shifted = targets[:, 1:].reshape(-1)
            B, S1 = targets[:, 1:].shape
            if self.use_focal_loss:
                loss_per_token = focal_cross_entropy(
                    logits_shifted, labels_shifted, gamma=self.focal_gamma, ignore_index=-100, reduction="none"
                )
            else:
                loss_per_token = F.cross_entropy(
                    logits_shifted, labels_shifted, ignore_index=-100, reduction="none"
                )
            loss_2d = loss_per_token.view(B, S1)
            mask = (targets[:, 1:] != -100)
            loss = (loss_2d * mask.float()).sum(dim=1) / mask.sum(dim=1).clamp(min=1).float()
    ...
    def compute_logits(self, samples, completion_ids):
        # detect whether there are multi tasks in this batch
        task = list(set(samples["task"]))
        if len(task) > 1 or "QA" in task:
            self.multi_prompt = True
        ...
        with self.maybe_autocast():
            ...
            # Note: outputs.logits includes prediction for the last token too.
    ...
    def generate(self, samples, generate_cfg, prompts=None):
        ...
        elif self.prompt_dict:
            # Mirror forward(): apply prompt_dict at inference so train/inference contexts match.
            # All samples share the same task ("antispoofing"), so pick one prompt per sample.
            task_keys = samples.get("task", ["antispoofing"])
            task_key = task_keys[0] if isinstance(task_keys, (list, tuple)) and task_keys else "antispoofing"
            if task_key not in self.prompt_dict:
                task_key = next(iter(self.prompt_dict))
            batch_prompts = [random.choice(self.prompt_dict[task_key]) for _ in range(batch_size)]
            speech_embeds, speech_atts = self.prompt_wrap(speech_embeds, speech_atts, batch_prompts, multi_prompt=True)
        ...
        try:
            ...
            with self.maybe_autocast():
                ...
                outputs = generation_model.generate(**gen_kwargs)
    ...
    def from_config(cls, config):
        ...
        model.use_focal_loss = use_focal_loss
